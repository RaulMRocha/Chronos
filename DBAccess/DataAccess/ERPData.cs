    ...
    public static class ERPData
    {
        ...
        public static WorkOrderHeader GetMFGOrderNumberInfoByOrder(IChronosGlobal chronosGlobal, string WorkTag)
        {
            string SelectOPTRunInfo = @"SELECT [M1RUN_RunNumber],[M1MWO_MFGOrderNumber],[M1ORD_SalesOrderNumber],[M1WH_Warehouse],[M1WHADDR_WarehouseAddress]
                                                    ,[N_ProductNumber],[M1DESC_ProductDescription],[M1OQ_OrderQuantity],[M1TOQ_TotalOrderQuantity],[M1STRT_ScheduledStartDate]
                                                    ,[M1DUE_ScheduledDueDate],[M1CRQD_CustomerRequestedDeliveryDate],[M1OPDT_OrigPrintTimeStamp],[RDT_PrintTimeStamp]
                                                    ,[M1BTKY_SoldToCustomer],[M1STKY_ShipToCustomer],[M1CMNAME_CustomerName],[RKY_PriceAsCustomer],[M1SPO_CustomerPO]
                                                ...
                                                FROM [Chronos].[dbo].[WorkOrderHeader] WITH(NOLOCK) 
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new WorkOrderHeader
                            {
                                ...
                                N_ProductNumber = reader["N_ProductNumber"].ToString(),
                                ...
                                RDT_PrintTimeStamp = reader["RDT_PrintTimeStamp"].ToString(),
                                ...
                                RKY_PriceAsCustomer = reader["RKY_PriceAsCustomer"].ToString(),
